...
pub fn validate_user(config: config::Config, user: String, ssh_key: String) -> bool {
	...
	let res = client.get(&format!("{}/access/{}/{}/{}?ref=build&&access_token={}", config.keyhouse_base_url, config.keyhouse_hostname, user, hex, config.keyhouse_token))
				.send();
...
pub fn get_name(config: config::Config, ssh_key: String) -> String {
	...
	hasher.input_str(&ssh_key);
	let hex = hasher.result_str();
	let mut res = reqwest::get(&format!("{}/names/{}?ref=build&access_token={}", config.keyhouse_base_url, hex, config.keyhouse_token)).unwrap();
