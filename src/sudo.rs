...
pub fn handle_sudo() -> Result<()> {
    info!(target: "sudo", "Handling sudo command");
    ...
    info!(target: "sudo", "PAM_RUSER: {}", pam_ruser);
    info!(target: "sudo", "PAM_TYPE: {}", pam_type);
    if pam_type == "open_session" {
        let config = read_config()?;
        init(&config)?;
        info!(target: "sudo", "Logging successful");
        match fork() {
